...
prototype.webLogOn = function(callback) {
  ...
  var cryptedSessionKey = Steam.pubKey.encrypt(sessionKey);
...
handlers[EMsg.ClientUpdateMachineAuth] = function(data, jobID) {
  var updateAuth = schema.CMsgClientUpdateMachineAuth.parse(data);
  var sha = require('crypto').createHash('sha1');
  sha.end(updateAuth.bytes.buffer.slice(updateAuth.bytes.offset, updateAuth.bytes.limit));
  sha = sha.read();
