...
Steam.pubKey = new NodeRSA(fs.readFileSync(__dirname + '/public.pub'));
Steam.pubKey.schemeEncryption = NodeRSA.RSA.OAEP.Default;
...
var schema = new ProtoBuf.newBuilder({ convertFieldsToCamelCase: true });
ProtoBuf.loadProtoFile(__dirname + "/../Protobufs/steamclient/steammessages_base.proto", schema);
ProtoBuf.loadProtoFile(__dirname + "/../Protobufs/steamclient/steammessages_clientserver.proto", schema);
ProtoBuf.loadProtoFile(__dirname + "/../Protobufs/steamclient/steammessages_clientserver_2.proto", schema);
schema = Steam.Internal = schema.build();

// Add serialize and parse functions to protobufjs objects for easy conversion from protobuf:
for(var msgName in schema){
	(function(msgName){
		
		var msg = schema[msgName];
		
		msg.serialize = function(object){
			for(var key in object)
				if(object[key] === undefined)
					delete object[key];
			
			return (new msg(object)).encode().toBuffer();
		};
		
		msg.parse = function(buffer){
...
SteamClient.prototype._netMsgReceived = function(data) {
  ...
  for(var name in EMsg)
	if(EMsg[name] == eMsg){
		this.emit("debug", "Received "+name+ " packet");
		break;
	}
...
//  var encRequest = schema.MsgChannelEncryptRequest.parse(data);
  ...
  var cryptedSessKey = Steam.pubKey.encrypt(this._tempSessionKey);
...
handlers[EMsg.Multi] = function(data) {
  ...
  var payload = msgMulti.messageBody.buffer.slice(msgMulti.messageBody.offset);
